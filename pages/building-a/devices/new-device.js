...
export async function getServerSideProps(context) {
  const building = context.query.b;
  const level = context.query.l;
  const output = parseInt(context.query.o);
  const deviceId = context.query.id;
  const res = await fetch(`http://127.0.0.1:3001/apis/devices/new`,
  {
    headers: {
